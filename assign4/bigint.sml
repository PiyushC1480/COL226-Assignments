...
fun charToInt c =
	...
	fun negate(str : bigint) : bigint = "-" ^ str
...
fun gusser(divisor:bigint,rem:bigint):bigint = 
	...
	fun quotient(a:bigint,b:bigint):bigint = 
		let 
			...
			val n = stripZeroes(abs(b))
		in
			if String.sub(a,0) = #"-" andalso String.sub(b,0) = #"-" then divider(abs(m),abs(n),true)
			else if (String.sub(a,0) = #"-" andalso String.sub(b,0) <> #"-" )orelse (String.sub(a,0) <> #"-" andalso String.sub(b,0) = #"-" ) then negate(add(divider(abs(m),abs(n),true),"1"))
			else divider(m,n,true)
		end
